class DropInsController < ApplicationController
  ...
  def create
    ...
    if retrieved_params[:user_id].blank?
      ...
      if @drop_in.save
        RetailUserMailer.drop_in_scheduled_email(@drop_in).deliver_later
        ShopperMailer.drop_in_scheduled_email(@drop_in).deliver_later
        AdminMailer.drop_in_scheduled(@drop_in).deliver_later unless User.admins.empty?
  ...
  def destroy
    RetailUserMailer.drop_in_canceled_email(@drop_in).deliver_later
    ShopperMailer.drop_in_canceled_email(@drop_in).deliver_later
    @drop_in.destroy
