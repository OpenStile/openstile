...
RSpec.describe DropInAvailabilitiesController, :type => :controller do
  ...
  let(:wrong_retail_user){ FactoryGirl.create(:retailer_user, retailer: other_retailer) }
  let(:non_retailer){ FactoryGirl.create(:shopper_user) }
  ...
  context "when non-retailer user is signed in" do
    before { sign_in non_retailer }
